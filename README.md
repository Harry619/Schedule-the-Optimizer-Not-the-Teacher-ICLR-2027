# Schedule-the-Optimizer-Not-the-Teacher-ICLR-2027
Code and data for "Schedule the Optimizer, Not the Teacher": diagnosing and mitigating long-horizon collapse in on-policy self-distillation (OPSD) of LLM reasoning.
...
ledger (280 records), per-problem scores for every evaluation, the canonical
...
2. **The cause is optimization-side, not teacher-side.** Periodic teacher
   ...
   $B=\int\eta\,dt$.
3. **An EMA teacher is a viable anchor-side alternative**, matching the best
   early score within budget.
...
├── analysis/
│   ├── compact_results.json         # full result ledger, 280 records (append-only)
│   ├── per_problem_scores.json      # per-problem correctness for all 128 evaluation files
│   ├── bootstrap_canonical.py       # reproduces ALL 22 confidence intervals in the paper
